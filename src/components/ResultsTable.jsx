...
import {MenuItem, Select, Link, Box, Grid2, List, Card, CardContent, Typography, ListItem, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper,
...
export default function ResultsTable({results}){
    ...
    // Loading more variants if no.of retrievable variants >= 2500
    const [loadMoreTimes, setLoadMoreTimes] = useState(1)

    // resets loadMoreTimes to 1 each a new query is searched
    useEffect(() => {
        setLoadMoreTimes(1)
    }, [results])

    console.log(loadMoreTimes)

    const loadMore = async () => {

        const newCount = loadMoreTimes + 1
        setLoadMoreTimes(newCount)
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({
                query: query,
                db_type: db,
                load_more_times: newCount
            })
        }

        try{

            const res = await api.post('/api/retrieve/', requestOptions)
            console.log(res)

            if (res.status == 200){

                const [moreTableRows, combinedData, tot_individuals] = createEntries(res)
                
                console.log(moreTableRows)

                setRows([...rows,...moreTableRows])
            ...
            alert("You have retrieved all possible variants")
            console.log(error)
        }
            ...
            <Grid2 container size = "grow"
                ...
                {rows ? 
                    ...
                    <Button
                        variant = 'contained'
                        sx = {{mt:2}}
                        onClick = {() => handleDownload(rows,combinedData,type)}
                    >
                        Download Results
                ...
                {/* Load more variants button if no. of variants >= 3000 */}
                {rows.length >= 2500 ?
